...
void BattlegroundQueue::BattlegroundQueueAnnouncerUpdate(uint32 diff, BattlegroundQueueTypeId bgQueueTypeId, BattlegroundBracketId bracket_id)
{
    ...
    {
        uint32 qPlayers = 0;
        uint32 qHordePlayers = 0;
        uint32 qAlliancePlayers = 0;
        ...
        else
        {
            ...
            qHordePlayers = GetPlayersCountInGroupsQueue(bracket_id, BG_QUEUE_NORMAL_HORDE);
            qAlliancePlayers =GetPlayersCountInGroupsQueue(bracket_id, BG_QUEUE_NORMAL_ALLIANCE);
        }
        ...
        if (_queueAnnouncementTimer[bracket_id] >= 0)
        {
            if (_queueAnnouncementTimer[bracket_id] <= static_cast<int32>(diff))
            {
                ...
                ChatHandler(nullptr).SendWorldTextOptional(LANG_BG_QUEUE_ANNOUNCE_WORLD_AH, ANNOUNCER_FLAG_DISABLE_BG_QUEUE, bgName.c_str(), q_min_level, q_max_level, qPlayers, MaxPlayers,qAlliancePlayers,qHordePlayers);
            }
            else
            {
...
void BattlegroundQueue::SendMessageBGQueue(Player* leader, Battleground* bg, PvPDifficultyEntry const* bracketEntry)
{
    ...
    else // Show queue status to server (when joining battleground queue)
    {
        ...
        else
        {
            ...
            ChatHandler(nullptr).SendWorldTextOptional(LANG_BG_QUEUE_ANNOUNCE_WORLD_AH, ANNOUNCER_FLAG_DISABLE_BG_QUEUE, bgName.c_str(), q_min_level, q_max_level, qAlliance + qHorde, MaxPlayers,qAlliancePlayers,qHordePlayers);
        }
    }
}
